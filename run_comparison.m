...
for i=1:numel(images)
    ...
    segPath = fullfile(outDir,[D(i).name(1:end-4) '.mat']);
    if ~exist(outPath,'file'), im2ucm(imPath, segPath);  end

    % 2. compute intrinsic image (Just use reflectance for the moment)
    albedoPath = fullfile(outDir,[D(i).name(1:end-4) '_albedo.mat']);
    if ~exist(albedoPath, 'file'), im2reflectance(imPath, albedoPath); end
